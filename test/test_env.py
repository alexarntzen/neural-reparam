...
class TestEnv(unittest.TestCase):
    ...
    def test_r_integral_real(self):
        ...
        for c in [c1, c2]:
            for _ in range(10):
                for N in [256, 1024]:
                    # data
                    env = RealReparamEnv(r_func=c.r, q_func=c.q, size=N)
                    # random index and diff
                    ...
                    # sum of partials for same approx
                    part_1 = env.cost(start, middle)
                    part_2 = env.cost(middle, end)
                    total = env.cost(start, end)
                    self.assertAlmostEqual(part_1 + part_2, total, delta=1e-3)
    ...
    def test_compare_local(self, check_time=False):
        ...
        for c in [c1, c2]:
            for N in [2 ** (n * 2) for n in range(3, 5)]:
                # random index
                ...
                env = DiscreteReparamEnv(data=data)
                env_real_data = RealReparamEnv(data=data)
                env_real = RealReparamEnv(r_func=c.r, q_func=c.q, size=N)

                # compute result
                r_eval = r_cost(state_index=start, next_state_index=end, env=env).item()
                r_eval_real = env_real.cost(x_train[start], x_train[end])
                r_eval_real_data = env_real_data.cost(x_train[start], x_train[end])
                print(f"r_real_interp_cost:{r_eval_real},")
                print(f"r_real_data_cost:{r_eval_real_data},")
                print(f"r_cost:{r_eval}")
                ...
                # compare two reals
                self.assertAlmostEqual(
                    r_eval_real, r_eval_real_data, delta=r_eval_real * 100 / N
                )
                print()
                if check_time:
            ...
            for _ in range(10):
                for N in [2 ** (n * 2) for n in range(5, 7)]:
                    ...
                    if c is c2:
                        ...
                        x_torch = torch.unsqueeze(
                            torch.tensor(x_train, requires_grad=True), 1
                        )

                        pl_value = penalty_free_loss_func(
                            path_ksi, x_torch, torch.as_tensor(q_train)
                        ).item()
                        compare_value = pl_value

                    print(f"r_cost: {value}, true cost: {compare_value}")
                    self.assertAlmostEqual(compare_value, value, delta=100 / np.log(n))
